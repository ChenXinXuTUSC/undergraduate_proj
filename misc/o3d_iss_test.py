...
if __name__ == "__main__":
    ...
    for points1, points2, T_gdth, sample_name in dataloader:
        ...
        # step2: detect key points using open3d
        keypts1 = np.asarray(o3d.geometry.keypoint.compute_iss_keypoints(utils.npy2o3d(points1)).points)
        keypts2 = np.asarray(o3d.geometry.keypoint.compute_iss_keypoints(utils.npy2o3d(points2)).points)
        keypts1 = np.hstack([keypts1, np.array([[255, 0, 255]]).repeat(len(keypts1), axis=0)])
        keypts2 = np.hstack([keypts2, np.array([[255, 0, 255]]).repeat(len(keypts2), axis=0)])
        keypts1 = np.hstack([keypts1, np.array([[0, 0, 0]]).repeat(len(keypts1), axis=0)])
        keypts2 = np.hstack([keypts2, np.array([[0, 0, 0]]).repeat(len(keypts2), axis=0)])

        out_dir  = "./samples/matches_sample"
        utils.fusexfrags([points1, points2, keypts1, keypts2], utils.ply_vertex_type, out_dir, "output.ply")
